...
import {ExpenseDatabaseFacade, makeRequest} from "~/expense/db_facade/facade";
...
describe("Integration test of the API facade for get_single", function () {
    ...
    it("should return an item for id 1", function (done) {
        new ExpenseDatabaseFacade().get_single(1).then((item) => {
            expect(item.id).toBe(1);
            done()
        }, () => {
            fail("get_single's promise was rejected");
        });
    });
